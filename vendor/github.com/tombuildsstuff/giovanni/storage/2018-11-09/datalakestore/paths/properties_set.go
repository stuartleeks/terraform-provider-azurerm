...
type SetPropertiesInput struct {
	// A map of base64-encoded strings to store as user-defined properties with the Path
...
// SetProperties sets the Properties for a Data Lake Store Gen2 Path within a Storage Account File System
func (client Client) SetProperties(ctx context.Context, accountName string, fileSystemName string, path string, input SetPropertiesInput) (result autorest.Response, err error) {
	if accountName == "" {
	...
	req, err := client.SetPropertiesPreparer(ctx, accountName, fileSystemName, path, input)
	if err != nil {
...
func (client Client) SetPropertiesPreparer(ctx context.Context, accountName string, fileSystemName string, path string, input SetPropertiesInput) (*http.Request, error) {
