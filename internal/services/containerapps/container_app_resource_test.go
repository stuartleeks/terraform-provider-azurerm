...
func TestAccContainerAppResource_completeVolumeSecrets(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_container_app", "test")
	r := ContainerAppResource{}

	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			Config: r.completeWithVolumeSecrets(data, "rev1"),
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
			),
		},
...
func (r ContainerAppResource) completeWithVolumeSecrets(data acceptance.TestData, revisionSuffix string) string {
	return fmt.Sprintf(`
%s
...
resource "azurerm_container_app" "test" {
  ...
  secret {
    name = "volume-secret2"
    value = "shh, this is a secret as well"
  }
